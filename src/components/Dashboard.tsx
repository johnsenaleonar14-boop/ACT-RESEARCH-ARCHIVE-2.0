...
export function Dashboard({ savedItems, toggleSave }: DashboardProps) {
  ...
  const filteredData = MOCK_DATA.filter(item => {
    ...
    const matchesStrand = filterStrand ? item.strand === filterStrand : true;
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      ...
      {/* Hero Search Section */}
      <div className="bg-[#1178ec] rounded-[20px] p-8 md:p-12 text-center text-white shadow-lg relative overflow-visible">
        ...
        <div className="max-w-3xl mx-auto flex flex-col md:flex-row gap-4 relative z-20" ref={searchContainerRef}>
          <div className="relative flex-1">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={20} />
            <input 
              type="text"
              placeholder="Search by title, keyword..."
              className="w-full pl-12 pr-4 py-4 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-400/30 shadow-sm bg-white cursor-text" 
              value={searchQuery}
              onChange={(e) => {
              ...
              onFocus={() => {
                if (searchQuery.length > 0) setShowAutocomplete(true);
              }}
            ...
            {showAutocomplete && autocompleteResults.length > 0 && (
              <ul className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 text-left max-h-[300px] overflow-y-auto">
                {autocompleteResults.map((item, index) => (
                  ...
                  >
                    <p className="font-semibold text-slate-800 text-sm truncate">
                        {item.title.replace(new RegExp(`(${searchQuery})`, 'gi'), (match: string) => `<mark>${match}</mark>`).split(/<mark>|<\/mark>/).map((part, i) => 
                            part.toLowerCase() === searchQuery.toLowerCase() ? <span key={i} className="text-blue-600 bg-blue-50 font-bold">{part}</span> : part
                        )}
                    </p>
                    <p className="text-xs text-slate-500">{item.strand} • {item.year}</p>
                  </li>
                ))}
              </ul>
            )}
          </div>
          <button className="bg-[#0a0da4] hover:bg-[#1a51de] text-white font-bold py-4 px-8 rounded-xl transition-colors shadow-md border border-white/20 cursor-pointer">
            Search
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-2">
          {filteredData.length > 0 ? (
            filteredData.map((item) => (
              <div key={item.id} className="grid grid-cols-12 gap-4 px-4 py-6 hover:bg-slate-50 rounded-lg transition-colors border-b border-slate-50 items-center group cursor-default">
                <div className="col-span-8 md:col-span-7">
                  <h4 className="font-semibold text-slate-800 text-lg leading-snug cursor-pointer hover:text-blue-600 transition-colors">{item.title}</h4>
                  <p className="text-slate-500 text-sm mt-1">{item.type}</p>
                </div>
                ...
                <div className="col-span-1 md:col-span-1 flex justify-center">
                  <button 
                    ...
                    className="p-2 rounded-full hover:bg-slate-200 transition-colors focus:outline-none cursor-pointer"
                  >
                    <Star 
                      size={20} 
